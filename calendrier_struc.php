...
$element_type_even=$req_type_even->fetchall();
    ...
    if ($_SESSION['poste']=='Directeur' || $_SESSION['poste']=='Sous-Directeur' || $_SESSION['poste']=='Chef de Service' || $_SESSION['poste']=='Secretaire' || $_SESSION['poste']=='Secretaire2')
    {
        ...
        $req_rdv1=$bdd->prepare("SELECT * FROM rdv,usager where usager.id_usager=rdv.id_usager and rdv.id_hira=? ");
        $req_rdv1->execute(array($_SESSION['id_hira']));
        $element_rdv1=$req_rdv1->fetchall();
        $nbre_rdv1 =$req_rdv1->rowcount();

        $req_event=$bdd->prepare("SELECT * FROM event,type_even,agent where agent.id_agent=event.id_agent and event.id_type_even=type_even.id_type_even and event.id_hira=? ");
                            ...
                            <div class="nk-content-body">
                                ...
                                <div class="nk-block">
                                    <div class="row g-gs">
                                        ...
                                        <form style="display:none;" action="calendrier_param_struc.php" method="POST" accept-charset="utf-8">
  ...
  opacity: 0.3;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: [
                <?php if($_SESSION['poste']=='Agent'){ $nbre_rdv1='0'; } if ($nbre_rdv1>'0'){ foreach($element_rdv1 as $elt_rdv1){ $start= new DateTime($elt_rdv1['date_rdv']) ?>
                    {   id     : '<?php echo $elt_rdv1['id_rdv']; ?>',
                        title  : 'Rendez-Vous',
                        start  : '<?php echo $elt_rdv1['date_rdv']." ".$elt_rdv1['heure_rdv']; ?>',
                        description : '<?php echo $elt_rdv1['heure_rdv']; ?>',
                        color  : '<?php echo 'blue'; ?>' },
                <?php } }?>
                <?php if ($nbre_event>'0'){ foreach($element_event as $elt_event){ $start= new DateTime($elt_event['start_event']); $end= new DateTime($elt_event['end_event']); if ($elt_event['start_event']==$elt_event['end_event']) {?>
